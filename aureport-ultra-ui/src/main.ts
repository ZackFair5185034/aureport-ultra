...
const uComponents: Record<string, any> = {
  UButton, UButtonGroup, UForm, UFormItem, URow, UCol,
  USelect, UOption, UInput, UInputNumber, USwitch,
  UCheckbox, UCheckboxGroup, URadio, URadioGroup, UDatePicker,
  UColorPicker, UDivider, ULoading, UDialog, UTabs, UTabPane,
  UTag, UTree, UMessage, UMessageBox
}
...
for (const [name, component] of Object.entries(uComponents)) {
  if (component) {
    ...
    const kebab = 'u-' + name.replace(/^U/, '').replace(/([A-Z])/g, '-$1').toLowerCase()
