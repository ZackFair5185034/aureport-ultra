...
<template>
  <div class="u-tabs tabs" :class="[type ? `-${type}` : '']">
    <div v-if="navPosition === 'bottom' || navPosition === 'right'" class="content" :style="contentStyle">
      <slot />
    </div>
    <ul class="nav" :class="[navPosition, { center: navCenter }]">
      <li
        v-for="pane in panes"
        :key="pane.index"
...
.u-tabs.tabs.-button .nav li {
  border: none !important;
  border-radius: 3px;
  margin: 0 0.357em !important;
}

.u-tabs.tabs.-button .nav li.active,
.u-tabs.tabs.-button .nav li:hover {
